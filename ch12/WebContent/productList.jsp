	...
	<c:if test="${list!=null }">
		<c:forEach items="${list}" var="p">
			<tr align="center">
				<td>${p.pno}</td>
				<td>
					<a href="FindProductServlet?pno=${p.pno }">${p.pname}</a>
				</td>
				<td>${p.price}</td>
				<td>${p.category}</td>
				<td>${p.num}</td>	
				<td>
					<a href="UpdateProductServlet?pno=${p.pno}">修改</a>
					<a href="DeleteProductServlet?pno=${p.pno}">删除</a>
				</td>			
			</tr>
		</c:forEach>
	</c:if>
